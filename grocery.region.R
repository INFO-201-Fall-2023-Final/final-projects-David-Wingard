...
pac.df <- region[census.regions$Division == "Pacific", ]
pac <- unique(pac.df$Postal.Code)

esc.df <- region[census.regions$Division == "East South Central", ]
esc <- unique(esc.df$Postal.Code)

wsc.df <- region[census.regions$Division == "West South Central", ]
wsc <- unique(wsc.df$Postal.Code)

mtn.df <- region[census.regions$Division == "Mountain", ]
mtn <- unique(mtn.df$Postal.Code)

neg.df <- region[census.regions$Division == "New England", ]
neg <- unique(neg.df$Postal.Code)

sat.df <- region[census.regions$Division == "South Atlantic", ]
sat <- unique(sat.df$Postal.Code)

wnc.df <- region[census.regions$Division == "West North Central", ]
wnc <- unique(wnc.df$Postal.Code)

enc.df <- region[census.regions$Division == "East North Central", ]
enc <- unique(enc.df$Postal.Code)

mat.df <- region[census.regions$Division == "Middle Atlantic", ]
mat <- unique(mat.df$Postal.Code)
...
for(i in 1:nrow(avg.price)) {
  ...
  if((any(str_detect(avg.price$area_name[i], se)) == TRUE) || avg.price$area_name[i] == "South East") {
    avg.price$Census.Division[i] = "South East"
    
  } else if ((any(str_detect(avg.price$area_name[i], pac)) == TRUE) || avg.price$area_name[i] == "Pacific") {
    ...
    avg.price$Census.Division[i] <- NA
  }
}
...
obesity_region <- left_join(obesity_region, census.regions, by = c('Name' = 'State'))
...
write.csv(avg.price, "avg.price11.20.csv", row.names = FALSE)

# OBESITY + GROCERY JOIN

obesity.price <- full_join(obesity_region, census.regions, by = c('Census.Division' = 'Division'))
